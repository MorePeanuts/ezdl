...
from mini_transformer.scratch.linear_model import MultiLinearRegression, MLPForClassification
...
def train_classifier(model, lr=0.1, num_epochs=10, resize=(28, 28), batch_size=256, eval_freq=128):
    ...
    eval_dataloader = DataLoader(eval_data, batch_size=batch_size, shuffle=False, drop_last=False)
...
def train_softmax_classifier():
    ...
    train_classifier(model, lr=0.1, num_epochs=10, batch_size=256, eval_freq=128)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Train a classification for FashionMNIST dataset with different models.'
    )
    parser.add_argument(
        'model', choices=['softmax', 'mlp', 'lenet', 'alexnet', 'vgg', 'nin', 'googlenet']
    )
    ...
    match args.model:
        ...
        case _:
            raise ValueError(f'Invalid task: {args.model}')
