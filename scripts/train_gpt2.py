...
from mini_transformer.trainer_seq2seq import train_gpt2_simple, train_gpt2_classifier_simple
...
def train_gpt2_for_generation():
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=2, shuffle=False, drop_last=False)
...
def train_gpt2_for_classification():
    ...
    model.eval()
    context_length = model.config.context_length  # type: ignore
...
def train_gpt2_instruction_fine_tuning():
    ...
    train_losses, eval_losses, tokens_seen = train_gpt2_simple(
        ...
        eval_freq=5,
        eval_iter=5,
        start_context=tokenizer.decode(test_dataset[0][0]),  # type: ignore
        tokenizer=tokenizer,
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='A script to train GPT2 model on different task.')
    parser.add_argument('task', choices=['generation', 'classification', 'instruction'])
