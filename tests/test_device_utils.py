...
def test_forward_consistency(model, input_ids):
    ...
    assert diff < 1e-3, f'Forward consistency failed with max diff {diff}'
...
def test_gradient_consistency(model, input_ids):
    ...
    loss_cpu = torch.nn.functional.cross_entropy(
        output_cpu.flatten(0, 1), input_ids[:, 1:].flatten().to(cpu)
    )
    loss_cpu.backward()
    optimizer_cpu.step()
    ...
    loss_gpu = torch.nn.functional.cross_entropy(
        output_gpu.flatten(0, 1), input_ids[:, 1:].flatten().to(gpu)
    )
    loss_gpu.backward()
    optimizer_gpu.step()
    grad_gpu = [p.grad.cpu().clone() for p in model_gpu.parameters() if p.grad is not None]
    ...
    assert max_grad_diff < 1e-3, f'Gradient consistency failed with max diff {max_grad_diff}'
