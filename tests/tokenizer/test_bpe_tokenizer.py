...
def test_bpe_tokenizer_simple_with_the_verdict_dataset(input_text, token_ids):
    ...
    assert len(tokenizer.vocab) == 1000, (
        f'Expected vocabulary size of 1000, got {len(tokenizer.vocab)}'
    )
    assert len(tokenizer.bpe_merges) == 742, (
        f'Expected BPE merges of 742, got {len(tokenizer.bpe_merges)}'
    )
...
@pytest.mark.xfail(
    reason='BPETOkenizerSimple removes whitespace at both ends of the string when encoding, while the tokenizer in tiktoken does not.'
)
def test_bpe_tokenizer_simple_with_spaces_at_both_ends(input_text):
    tokenizer = BPETokenizerSimple()
    gpt2_dir = Path(__file__).parents[2] / 'models/gpt2'
    tokenizer.load_vocab_and_merges_from_gpt2(gpt2_dir / 'vocab.json', gpt2_dir / 'merges.txt')
    token_ids = tokenizer.encode(input_text, allowed_special={'<|endoftext|>'})
    decoded_text = tokenizer.decode(token_ids)
