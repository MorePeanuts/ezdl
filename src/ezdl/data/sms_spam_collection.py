...
class SMSSpamCollection(Dataset):
    ...
    dataset_path = Path(__file__).parents[3] / 'datasets/sms_spam_collection/sms_spam_collection.tsv'
    zipfile_path = Path(__file__).parents[3] / 'datasets/sms_spam_collection/sms_spam_collection.zip'
    dataset_url = (
        'https://archive.ics.uci.edu/'
        'static/public/228/sms+spam+collection.zip'
    )
    label_map = {
        'ham': 0,
        'spam': 1
    ...
    def get_raw_dataframe():
        ...
        return pd.read_csv(SMSSpamCollection.dataset_path, sep='\t', header=None, names=['label', 'text'])
