...
class MultiHeadAttention(nn.Module):
    def __init__(
        self, d_in, d_out, context_length, dropout, num_heads, qkv_bias=False, kv_window_size=None
    ):
        super().__init__()
    ...
    def forward(self, x, use_cache=False):
        ...
        if use_cache:
            if self.cache_k is None or self.cache_k.shape[0] != batch_size:
                self.cache_k = torch.zeros(
                    batch_size, self.num_heads, self.kv_window_size, self.head_dim, device=x.device
                )
        ...
        context_vecs = context_vecs.contiguous().view(batch_size, seq_len_q, self.d_out)
