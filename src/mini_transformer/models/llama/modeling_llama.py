...
class LlamaMLP(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.gate_proj = nn.Linear(
            in_features=self.hidden_size, out_features=self.intermediate_size, bias=self.mlp_bias
        )
        self.up_proj = nn.Linear(
            in_features=self.hidden_size, out_features=self.intermediate_size, bias=self.mlp_bias
        )
        self.down_proj = nn.Linear(
            in_features=self.intermediate_size, out_features=self.hidden_size, bias=self.mlp_bias
        )
