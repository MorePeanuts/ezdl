            ...
            if global_step % eval_freq == 0 or global_step == total_steps:
                ...
                with torch.no_grad():
                    ...
                    eval_loss = calc_loss_dataloader(
                        eval_dataloader, model, device, num_batches=eval_iter
                    )
...
def train_gpt2_classifier_simple(
    model, train_dataloader, eval_dataloader, optimizer, device, num_epochs, eval_freq, eval_iter
):
    from .models.gpt2 import calc_loss_batch, calc_loss_dataloader, calc_accuracy_dataloader
    ...
    for epoch in range(num_epochs):
        ...
        for input_batch, target_batch in train_dataloader:
            ...
            if global_step % eval_freq == 0 or global_step == total_steps:
                ...
                with torch.no_grad():
                    train_loss = calc_loss_dataloader(
                        train_dataloader, model, device, eval_iter, True
                    )
                    eval_loss = calc_loss_dataloader(
                        eval_dataloader, model, device, eval_iter, True
                    )
                train_losses.append(train_loss)
                eval_losses.append(eval_loss)
                pbar.write(
        ...
        model.eval()
        train_acc = calc_accuracy_dataloader(train_dataloader, model, device, eval_iter)
        eval_acc = calc_accuracy_dataloader(eval_dataloader, model, device, eval_iter)
        pbar.write(
